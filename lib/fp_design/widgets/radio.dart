...
const double _kOuterRadius = 12.0; // MODIFIED: 8.0 -> 12
const double _kInnerRadius = 12.0; // MODIFIED: 4.5 -> 12

/// This a copied filed from Flutter Material SDK, with very minor modifications.
/// Search for "MODIFIED" comments to see what was changed.
/// If you need to update to a later version provided by Flutter, replace this file
/// with the latest version and manually reapply the changes.

/// A material design radio button.
///
/// Used to select between a number of mutually exclusive values. When one radio
/// button in a group is selected, the other radio buttons in the group cease to
/// be selected. The values are of type `T`, the type parameter of the [FpRadio]
...
class FpRadio<T> extends StatefulWidget {
  ...
  @override
  _FpRadioState<T> createState() => _FpRadioState<T>();
}

class _FpRadioState<T> extends State<FpRadio<T>>
  ...
  @override
  void didUpdateWidget(FpRadio<T> oldWidget) {
...
class _RadioPainter extends ToggleablePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint = Paint()
      ...
      ..strokeWidth = 1.0; // MODIFIED: strokeWidth 2.0 -> 1.0
    canvas.drawCircle(center, _kOuterRadius, paint);

    // Inner circle and checkmark
    if (!position.isDismissed) {
      ...
      final path = Path();
      final origin = center.translate(-5.25, 0);
      path.moveTo(origin.dx, origin.dy);
      path.relativeLineTo(3.5, 3.5);
      path.relativeLineTo(7, -8);
